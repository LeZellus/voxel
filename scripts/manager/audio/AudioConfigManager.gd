...
# Configuration globale des volumes (0.0 à 1.0)
var audio_settings = {
	"master": 1.0,
	"sfx": 1.0,
	"music": 0.8,
	...
	# Volumes spécifiques par type de son
	"footsteps": 0.7,
	"actions": 0.8,
	"ui": 1.0,
	"environment": 0.6
...
# Catégories de sons avec leurs bus audio
...
func set_volume_for_category(category: String, volume: float):
	...
	# Notifier tous les managers que le volume a changé
...
func get_base_volume_for_category(category: String) -> float:
	if category in sound_categories:
		return sound_categories[category]["base_volume"]
	return 1.0
